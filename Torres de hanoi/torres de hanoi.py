...
var objects = (function() {
    ...
    var Disk = sprites.extend(sprites.Sprite, {
        init: function(diskObject) {
            ...
            this.release = function() {
                this.dragging = false;
                if (draggingEnabled){
                    //make sure the moved disk came from the top of its stack
    ...
    var Peg = sprites.extend(sprites.Sprite, {
        ...
        draw: function() {
            textAlign(CENTER, CENTER);
            stroke(lerpColor(0, 0, 0));
            fill(149, 149, 149);    // dark gray, peg color from Tom Cormens images
            rect(this.x, this.y, this.w, this.h, 3);
    ...
    for (var i = numberOfDisks - 1; i >= 0; i--) {
        ...
        // like {disk1:0, disk2:1}, where pegs are 0-indexed but disks arent
